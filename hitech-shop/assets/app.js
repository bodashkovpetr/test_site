...
(function () {
  ...
  // ---------- Унифицированное извлечение ошибок ----------
  function extractErrors(data, status) {
    const msgs = [];
    if (data) {
      if (Array.isArray(data.errors)) {
        for (const e of data.errors) {
          if (e?.msg) msgs.push(e.msg);
    ...
    if (!msgs.length && status) msgs.push(`HTTP ${status}`);
    return msgs.join('\n');
  }

  // ---------- API fetch wrapper (читает тело при ошибках и кидает исключение) ----------
  async function apiFetch(path, options = {}) {
    ...
    const text = await resp.text();       // тело читаем один раз
    let json = null;
    try { json = text ? JSON.parse(text) : null; } catch {}

    if (!resp.ok) {
      const err = new Error(extractErrors(json, resp.status));
  ...
  async function addToCart(id) {
    ...
    try {
      ...
      alert(e.message || 'Не удалось добавить в корзину');
    }
  }
  ...
  async function removeFromCart(id) {
    ...
    try {
      await apiFetch(`/cart/by-product/${id}`, { method: 'DELETE' });
      updateCartCountUI();
      return [];
    } catch (e) {
      alert(e.message || 'Не удалось удалить из корзины');
      return [];
    }
  }
  ...
  async function register(name, email, password) {
    ...
    // ожидаемый формат: { data: { user, token? } } или { user, token? }
    const payload = data?.data || data || {};
    if (payload.token) setTokenRaw(rawFromBearer(payload.token));
    if (payload.user) {
      setCurrentUser({ name: payload.user.name, email: payload.user.email, phone: payload.user.phone || '', card: '' });
    }
  ...
  async function updateProfile(partial) {
    ...
    try {
      ...
      const merged = Object.assign({}, user, data?.data || data || {});
      setCurrentUser(merged);
      updateAuthUI();
      return true;
    } catch (e) {
      alert(e.message || 'Не удалось обновить профиль');
      return false;
    }
  }
  ...
  async function checkout() {
    ...
    try {
      const user = getCurrentUser();
      if (!user) return { success: false, needLogin: true };
      const data = await apiFetch('/orders', { method: 'POST', body: JSON.stringify({}) });
      updateCartCountUI();
      return { success: true, order: (data?.data || data || {}).order };
    } catch (e) {
      return { success: false, error: e.message || 'Покупка не удалась' };
    }
  }
  ...
  async function renderOrders(containerId) {
    ...
    // Пробуем несколько путей, пока не получится
    let j = null;
    const headers = { ...authHeaders?.() };

    const tryGet = async (p) => {
      try { return await apiFetch(p, { method: 'GET', headers }); } catch { return null; }
    };

    j = await tryGet('/orders');
    if (!j) j = await tryGet('/user/orders');
    if (!j) j = await tryGet('/orders/my');

    if (!j) { el.innerHTML = '<p>Сеть недоступна или заказы недоступны.</p>'; return; }
    ...
    const fmtRub = c => ((+c||0)/100).toFixed(2) + ' ₽';
    ...
    el.innerHTML = orders.map(o => {
      ...
      const total = fmtRub(o.total_cents || o.totalCents || 0);
  ...
  // ---------- Вспомогательные инициализаторы форм (по желанию подключайте на страницах) ----------
  function initRegisterForm(formId = 'registerForm', errorBoxId = 'registerError', successRedirect = '/login.html') {
    const form = document.getElementById(formId);
    if (!form) return;
    const errBox = document.getElementById(errorBoxId);
    const showErr = (m) => {
      if (errBox) { errBox.textContent = m; errBox.style.display = 'block'; }
      else { alert(m); }
    };
    ...
    form.onsubmit = async (e) => {
      ...
      try {
        ...
        // Если бэкенд сразу выдал токен — можете отправлять на аккаунт
        if (getCurrentUser()) window.location.href = '/account.html';
        else window.location.href = successRedirect;
      } catch (e2) {
        showErr(e2.message || 'Не удалось зарегистрировать');
        console.error('Register error:', e2);
      }
    };
  }

  function initLoginForm(formId = 'loginForm', errorBoxId = 'loginError', successRedirect = '/account.html') {
    const form = document.getElementById(formId);
    if (!form) return;
    const errBox = document.getElementById(errorBoxId);
    const showErr = (m) => { if (errBox) { errBox.textContent = m; errBox.style.display = 'block'; } else { alert(m); } };
    const clearErr = () => { if (errBox) { errBox.textContent = ''; errBox.style.display = 'none'; } };
    ...
    form.onsubmit = async (e) => {
      ...
      if (ok) window.location.href = successRedirect;
      else showErr('Неверный email или пароль');
    };
  }
  ...
  window.app = {
    ...
    // ui
    initHeader,
    // формы (подключайте на нужных страницах)
    initRegisterForm, initLoginForm
  };
})();